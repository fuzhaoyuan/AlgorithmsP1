...
public class BruteBackup {
    private int numberOfSegments = 0;
    private LineSegment[] segments;

    public BruteBackup(Point[] points){
        ...
        for(int one = 0; one < points.length-3; one++){
            ...
            for(int two = one+1; two < points.length-2; two++){
                if(points[two] == points[one]) throw new IllegalArgumentException();
                exam[1] = points[two];
                for(int three = two+1; three < points.length-1; three++){
                    if(points[three] == points[one] || points[three] == points[two]) throw new IllegalArgumentException();
                    exam[2] = points[three];
                    for(int four = three+1; four < points.length; four++){
                        if(points[four] == points[three] || points[four] == points[two] || points[four] == points[one]) throw new IllegalArgumentException();
                        exam[3] = points[four];
                        //examine 4 points
...
//                                else if(exam[0].compareTo(exam[n]) > 0) min = n;
    ...
    private boolean allEqual(double[] a){
        int match = 0;
